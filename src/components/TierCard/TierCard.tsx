...
export const TierCard = forwardRef<HTMLDivElement, TierCardProps>(
  ({ data, nickname, button }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={`w-[420px] bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 rounded-2xl overflow-hidden shadow-2xl`}
      >
        {/* Top Border Gradient */}
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex-1">
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-1">Player</p>
              <h2 className="text-2xl font-bold text-white">{nickname}</h2>
              <div className="flex items-center gap-2 mt-2">
                <span className={`px-3 py-1 rounded-full text-sm font-bold bg-gradient-to-r ${style.gradient} text-gray-900`}>
                  {button}B
                </span>
          ...
          <div className="mt-5 pt-4 border-t border-gray-800 flex items-center justify-between">
            <p className="text-[10px] text-gray-600">V-ARCHIVE TIER CARD</p>
            <p className="text-[10px] text-gray-600">v-archive.net</p>
          </div>
        </div>
      </div>
    );
